...
export const ApiKeyCreateForm = ({
  ...
  >(null);
  const [showRedactedKey, setShowRedactedKey] =
    useState(true);
  ...
  const form = useForm<
    zod.infer<typeof ApiKeyCreateSchema>
  >({
    defaultValues: {
      title: '',
    },
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    await mutateAsync(
      ...
      { title: values.title },
      {
        onSuccess: ({ api_key }) => {
          toast.success(
            t('apiKeyManagement.create.successToast')
          );

          switch (keyType) {
            case ApiKeyType.PUBLISHABLE:
              handleSuccess(
                `/settings/publishable-api-keys/${api_key.id}`
              );
              break;
  ...
  const handleCopyToken = () => {
    if (!createdKey) {
      toast.error(
        t('apiKeyManagement.create.copySecretTokenFailure')
      );
    }

    navigator.clipboard.writeText(createdKey?.token ?? '');
    toast.success(
      t('apiKeyManagement.create.copySecretTokenSuccess')
    );
  };
  ...
  const handleGoToSecretKey = () => {
    ...
    handleSuccess(
      `/settings/secret-api-keys/${createdKey.id}`
    );
  };
                ...
                <div>
                  <RouteFocusModal.Title asChild>
                    <Heading>
                      {keyType === ApiKeyType.PUBLISHABLE
                        ? t(
                            'apiKeyManagement.create.createPublishableHeader'
                          )
                        : t(
                            'apiKeyManagement.create.createSecretHeader'
                          )}
                    </Heading>
                  ...
                  <RouteFocusModal.Description asChild>
                    ...
                    >
                      {keyType === ApiKeyType.PUBLISHABLE
                        ? t(
                            'apiKeyManagement.create.createPublishableHint'
                          )
                        : t(
                            'apiKeyManagement.create.createSecretHint'
                          )}
                    </Text>
      ...
      <Prompt variant='confirmation' open={!!createdKey}>
        <Prompt.Content className='w-fit max-w-[42.5%]'>
          <Prompt.Header>
            <Prompt.Title>
              {t(
                'apiKeyManagement.create.secretKeyCreatedHeader'
              )}
            </Prompt.Title>
            <Prompt.Description>
              {t(
                'apiKeyManagement.create.secretKeyCreatedHint'
              )}
            </Prompt.Description>
          </Prompt.Header>
