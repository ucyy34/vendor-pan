...
export const RulesFormField = ({
  ...
  const { rules, isLoading } = usePromotionRules(
    ...
    query,
    {
      enabled:
        !!promotion?.id ||
        (!!promotionType && !!applicationMethodType),
    }
  );
  ...
  return (
    <div className='flex flex-col'>
      ...
      {fields.map((fieldRule: any, index) => {
        ...
        return (
          <Fragment
            key={`${fieldRule.id}.${index}.${fieldRule.attribute}`}
          >
            <div className='bg-ui-bg-subtle border-ui-border-base flex flex-row gap-2 rounded-xl border px-2 py-2'>
              <div className='grow'>
                <Form.Field
                  ...
                  render={({ field }) => {
                    const { onChange, ref, ...fieldProps } =
                      field;

                    const existingAttributes =
                      fields?.map(
                        (field: any) => field.attribute
                      ) || [];
                    const attributeOptions =
                      attributes?.filter((attr) => {
                        if (
                          attr.value === fieldRule.attribute
                        ) {
                          return true;
                        }

                        return !existingAttributes.includes(
                          attr.value
                        );
                      }) || [];
                    ...
                    const onValueChange = (e: string) => {
                      ...
                      update(index, {
                        ...
                        values: [],
                        disguised:
                          currentAttributeOption?.disguised ||
                          false,
                      });
                      onChange(e);
                    };
 ...
 */
const DisabledField = forwardRef<
  HTMLInputElement,
  DisabledAttributeProps
>(({ label, field }, ref) => {
  return (
    <div>
