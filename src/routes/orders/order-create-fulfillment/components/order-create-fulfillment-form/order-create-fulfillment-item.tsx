...
export function OrderCreateFulfillmentItem({
  ...
  const { availableQuantity, inStockQuantity } =
    useMemo(() => {
      ...
      const locationInventory =
        inventory?.[0]?.location_levels?.find(
      ...
      const reservedQuantityForItem =
        itemReservedQuantitiesMap.get(item.id) ?? 0;
      ...
      <div className='flex flex-row items-center'>
        {disabled && (
          <div className='inline-flex items-center ml-4'>
            <Tooltip
              content={t(
                'orders.fulfillment.disabledItemTooltip'
              )}
              side='top'
            >
