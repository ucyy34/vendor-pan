...
export const PriceListConfiguration = () => {
  ...
  const { price_list, isPending, isError, error } =
    usePriceList(id!);

  const customerGroupIds = price_list?.rules?.[
    'customer.groups.id'
  ] as string[] | undefined;
  ...
  const ready =
    !isPending && !!price_list && isCustomerGroupsReady;
    ...
    <RouteDrawer>
      <RouteDrawer.Header>
        <RouteDrawer.Title asChild>
          <Heading>
            {t('priceLists.configuration.edit.header')}
          </Heading>
        </RouteDrawer.Title>
      </RouteDrawer.Header>
      {ready && (
