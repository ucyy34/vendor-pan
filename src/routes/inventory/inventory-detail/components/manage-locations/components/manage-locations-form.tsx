...
export const ManageLocationsForm = ({
  ...
  const form = useForm<
    zod.infer<typeof EditInventoryItemAttributesSchema>
  >({
    ...
    resolver: zodResolver(
      EditInventoryItemAttributesSchema
    ),
  });

  const { fields: locationFields, update: updateField } =
    useFieldArray({
  ...
  const { mutateAsync } =
    useBatchInventoryItemLocationLevels(item.id);

  const handleSubmit = form.handleSubmit(
    async ({ locations }) => {
      // Changes in selected locations
      const [selectedLocations, unselectedLocations] =
        locations.reduce(
          (acc, location) => {
            // If the location is not changed do nothing
            if (
              (!location.selected &&
                !existingLocationLevels.has(
      ...
      await mutateAsync(
        ...
        },
        {
          onSuccess: () => {
            toast.success(
              t('inventory.toast.updateLocations')
            );
            handleSuccess();
          },
          onError: (e) => {
