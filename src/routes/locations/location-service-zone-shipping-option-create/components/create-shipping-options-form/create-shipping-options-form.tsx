...
export function CreateShippingOptionsForm({
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    const currencyPrices = Object.entries(
      ...
      .filter(
        (
          p
        ): p is { currency_code: string; amount: number } =>
          !!p
      );
  ...
  const onTabChange = (tab: Tab) => {
    if (tab === Tab.PRICING) {
      ...
      const result =
        CreateShippingOptionDetailsSchema.safeParse({
          ...form.getValues(),
        });
            ...
            <ProgressTabs.List className='border-ui-border-base -my-2 ml-2 min-w-0 flex-1 border-l'>
              ...
              >
                <span className='w-full cursor-auto overflow-hidden text-ellipsis whitespace-nowrap'>
                  {t(
                    'stockLocations.shippingOptions.create.tabs.details'
                  )}
                </span>
              </ProgressTabs.Trigger>
              {!isCalculatedPriceType && (
                ...
                >
                  <span className='w-full overflow-hidden text-ellipsis whitespace-nowrap'>
                    {t(
                      'stockLocations.shippingOptions.create.tabs.prices'
                    )}
                  </span>
                </ProgressTabs.Trigger>
              )}
