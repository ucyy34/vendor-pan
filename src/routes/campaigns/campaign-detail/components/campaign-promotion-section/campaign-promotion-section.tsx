...
export const CampaignPromotionSection = ({
  ...
  const { promotions, count, isLoading, isError, error } =
    usePromotions({
      ...searchParams,
    });
  ...
  const { mutateAsync } = useAddOrRemoveCampaignPromotions(
    campaign.id
  );
  ...
  const handleRemove = async () => {
    ...
    const res = await prompt({
      ...
      description: t(
        'campaigns.promotions.remove.description',
        {
          count: keys.length,
        }
      ),
  ...
  return (
    <Container className='divide-y p-0'>
      <div className='flex items-center justify-between px-6 py-4'>
        ...
        <Link
          to={`/campaigns/${campaign.id}/add-promotions`}
        >
      ...
      <_DataTable
        ...
        noRecords={{
          message: t(
            'campaigns.promotions.list.noRecordsMessage'
          ),
        }}
      />
    </Container>
...
const PromotionActions = ({
  ...
  const { t } = useTranslation();
  const { mutateAsync } =
    useAddOrRemoveCampaignPromotions(campaignId);
